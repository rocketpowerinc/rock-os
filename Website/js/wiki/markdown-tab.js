...
import { enhanceCodeBlocks, enhanceInlineCode } from './code-blocks.js';
...
const tab = {
    key: config.key,
    label: config.label,
    emptyLabel: config.emptyLabel || `${config.label.toLowerCase()} files`,
    stateStorageKey: config.stateStorageKey || `rock-os-${config.key}-state`,
    pinnedDocsStorageKey: config.pinnedDocsStorageKey || `rock-os-${config.key}-pinned-docs`,
    searchStatusId: config.searchStatusId,
    searchInputId: config.searchInputId,
    refreshButtonId: config.refreshButtonId,
    indexUrl: config.indexUrl,
    docApiUrl: config.docApiUrl,
    searchApiUrl: config.searchApiUrl,
    pathPrefix: config.pathPrefix,
    directOpenPageName: config.directOpenPageName || `${config.key}.html`
};

const expandedFolders = new Set();
const stateStorageKey = tab.stateStorageKey;
const pinnedDocsStorageKey = tab.pinnedDocsStorageKey;
...
function loadSavedState() {
    ...
    try {
        ...
        console.warn(`Could not load ${tab.key} state:`, err);
    }
}
...
function saveState() {
    ...
    try {
        ...
        console.warn(`Could not save ${tab.key} state:`, err);
    }
}
...
function renderWelcomeState() {
    ...
    content.innerHTML = `
        <h1>Rock-OS ${escapeHtml(tab.label)}</h1>
...
function renderWikiError(title, message, details = []) {
    ...
    if (sidebar) {
        ...
        empty.className = 'wiki-empty-state';
        empty.innerText = `${tab.label} server is not available.`;
...
function folderPathsForDoc(path) {
    ...
    const parts =
        path
            .replace(new RegExp(`^${escapeRegExp(tab.pathPrefix)}/`), '')
            .split('/');
...
async function openDocFromUrlIfNeeded() {
    ...
    if (!allMarkdownFiles.includes(doc)) {
        ...
        renderWikiError(
            `${tab.label} document not found`,
            `The URL points to ${doc}, but that file is not in ${tab.indexUrl}.`,
            [
                `Check that the file exists under ${tab.pathPrefix}.`,
                `Click the refresh button after adding new ${tab.label.toLowerCase()} files.`
            ]
        );
...
async function loadIndex() {
    ...
    try {
        ...
        if (isDirectFileOpen()) {
            ...
            renderWikiError(
                `Start the Rock-OS ${tab.label} server`,
                `This page was opened directly from the filesystem, so the browser cannot safely load the ${tab.key} index or files.`,
                [
                    'Open a terminal in the repo root or Website folder.',
                    'Run the Go server command below.',
                    `Use the http:// address printed by the server instead of opening ${tab.directOpenPageName} directly.`
                ]
            );
        ...
        if (!response.ok) {
            ...
            renderWikiError(
                ...
                `The page loaded, but ${tab.indexUrl} was not available from the server.`,
                [
        ...
        renderWikiError(
            `Could not connect to the ${tab.key} server`,
            `The browser could not load the ${tab.key} index. The server may not be running, or the page may have been opened from the wrong place.`,
            [
...
const refreshButton =
    document.getElementById(tab.refreshButtonId);

const toggleAllFoldersBtn =
    document.getElementById('toggleAllFoldersBtn');

const searchInput =
    document.getElementById(tab.searchInputId);
